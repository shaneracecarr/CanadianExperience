            ...
            wxBORDER_SIMPLE)
{
    ...
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewTimeline::OnEditMachineStartTimes, this, XRCID("EditMachineStartTimes"));
 ...
 * @param event The menu event
 */
void ViewTimeline::OnEditMachineStartTimes(wxCommandEvent &event)
{
    MachineStartDlg dlg(this->GetParent(), GetPicture()->GetTimeline());
    if(dlg.ShowModal() == wxID_OK)
    {
        GetPicture()->UpdateObservers();
    }
